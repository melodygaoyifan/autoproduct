...
def run_correction(
    ...
    *,
    ...
    # suite must pass, committed and visible.
    spec = load_spec(root, slug)
    sources = {}
    design_files = [
        f for f in yaml.safe_load(
            (root / "specs" / slug / "spec.yaml").read_text(encoding="utf-8")
        ).get("design", "").split()
        if "." in f
    ]
    ...
    try:
        ...
        written = _write_files(root, data.get("files") or [], allowed_test_paths=allowed_tests)
    except ValueError as exc:
        return CorrectionResult(status="error", spec_slug=slug, kind=kind, detail=str(exc))
    if not written:
    ...
    if report.status in ("failed", "error"):
        ...
        return CorrectionResult(
            ...
            detail=f"repair broke the suite ({report.summary}); reverted",
        )
    ...
    return CorrectionResult(status="fixed", spec_slug=slug, kind=kind,
                            commit=sha, detail=f"repaired; files: {', '.join(written)}")
